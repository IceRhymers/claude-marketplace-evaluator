...
def _make_plugin(name: str) -> PluginInfo:
    return PluginInfo(name=name, root_dir=Path(f"/fake/{name}"), skills_dir=Path(f"/fake/{name}/skills"))


def test_filter_single_pattern_match() -> None:
    plugins = [_make_plugin("git-commit"), _make_plugin("slack-notify"), _make_plugin("git-pr")]
...
def test_filter_multi_pattern_or_match() -> None:
    plugins = [_make_plugin("git-commit"), _make_plugin("slack-notify"), _make_plugin("jira-create")]
